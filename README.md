...
## 最小验活（Reward 闭环）
可用 `smoke` 查询参数快速做一轮可重复验活：

1. 打开 `http://localhost:8080/?smoke=1`。
2. 点击“开始行动”，在小队页任选 1 个节点协议并点击“部署到节点”。
3. 战斗页执行一次“攻击”（`smoke=1` 下首节点敌方完整性固定为 1）。
4. 验证进入奖励页后先点选奖励，再点击底部“安装：xxx”；再次点同一卡片应可取消锁定。
5. 验证成功推进到下一节点（回到小队页，节点进度进入 `2/4`）。
6. 可顺手验证键盘可达：奖励卡支持 `Enter/Space` 选中，战斗页支持 `A/D/S/B/R` 快捷操作。
